...
const fetcher = (url) =>
  fetch(url)
    .then((res) => {
      if (!res.ok) throw new Error("Erreur lors de la récupération des accessoires");
      return res.json();
    })
...
const getKey = (pageIndex, previousPageData) => {
  ...
  return `/api/products?page=${pageIndex + 1}&pageSize=${PAGE_SIZE}&category=ACCESSORY`; // 👈 filtre accessoires
};
    ...
    <div>
      {error && (
        <p className="text-center text-red-500 font-semibold py-4">
          {error.message || "Impossible de charger les accessoires."}
        </p>
      )}
...
export default React.memo(AccessoriesList);
